    ...
    pub fn new(
        ...
        let build_ops_content: Rc<dyn Fn()> = {
            ...
            Rc::new(move || {
                ...
                let has_music = if crate::music_operations::is_dismissed() {
                    if let Some(pending) = build_music_pending_widget() {
                        top_slot.append(&pending);
                        true
                    } else {
                        false
                    }
                } else {
                    let ts = top_slot.clone();
                    let on_stop = move || {
                        while let Some(c) = ts.first_child() {
                            ts.remove(&c);
                        }
                        if let Some(pending) = build_music_pending_widget() {
                            ts.append(&pending);
                    ...
                    if let Some(card) = now_playing_widget(on_stop) {
                        top_slot.append(&card);
                        true
                    } else {
                        false
                    }
        ...
        let search_debounce_id: Rc<Cell<Option<glib::SourceId>>> = Rc::new(Cell::new(None));
        {
            ...
            entry.connect_changed(move |e| {
                ...
                *results.borrow_mut() = new.clone();
                ...
                // Music search results also get an overview preview pane.
                let has_music = new
                    .iter()
                    .any(|r| matches!(&r.action, Action::PlayMusic { .. }));
                preview_box.set_visible(has_files || has_music);
                for r in &new {
            ...
            .build();
        {
            ...
            kc.connect_key_pressed(move |_, key, _, state| {
                ...
                // Undo — resume a music session dismissed from Operations,
                // but only while the Operations popover itself is open.
                if undo_hit
                    && ops_popover_kc.is_visible()
                    && crate::music_operations::is_dismissed()
                {
                    if crate::music_operations::restore() {
                        return glib::Propagation::Stop;
                    }
                }
                ...
                match key {
                    ...
                    Key::Left => {
                        if ops_popover_kc.is_visible() && crate::music_operations::current().is_some() {
                            crate::music_operations::seek(-5_000);
                            return glib::Propagation::Stop;
                        }
                    ...
                    Key::Return | Key::KP_Enter => {
                        ...
                        if let Some(row) = l.selected_row() {
                            ...
                            if let Some(res) = rs.get(row.index() as usize) {
                                ...
                                // The now-playing music row: Enter toggles
                                // play/pause (transport buttons handle skips).
                                if res.icon.as_deref() == Some("music-row") {
                                    crate::music_operations::toggle();
                                    return glib::Propagation::Stop;
                                }
...
fn build_music_undo_bar_widget() -> gtk::Box {
    let bar = gtk::Box::builder()
        .orientation(gtk::Orientation::Horizontal)
        .spacing(8)
        .margin_start(6)
    ...
    let lbl = gtk::Label::builder()
        .label("Music paused — swipe back or Ctrl+Z to resume")
        .hexpand(true)
        .xalign(0.0)
        .css_classes(["caption"])
        .build();
    let btn = gtk::Button::builder()
        ...
        .build();
    btn.connect_clicked(move |_| {
        crate::music_operations::restore();
    });
    bar.append(&lbl);
    bar.append(&btn);
    bar
...
fn build_music_card_snapshot() -> Option<gtk::Box> {
    let op = crate::music_operations::current()?;
    let track = op.current_track()?.clone();
    ...
    let cover = gtk::Image::builder().pixel_size(48).build();
    match &op.cover_path {
        Some(p) if p.exists() => cover.set_from_file(Some(&p.display().to_string())),
        _ => cover.set_icon_name(Some("audio-x-generic-symbolic")),
    }
    cover.add_css_class("music-cover");
    top.append(&cover);
    ...
    textbox.append(
        &gtk::Label::builder()
            .label(format!(
                "{} · {} / {}",
                artist,
                fmt_clock(op.elapsed_ms),
                fmt_clock(op.duration_ms)
            ))
            .xalign(0.0)
            .css_classes(["dim-label", "caption"])
            .ellipsize(gtk::pango::EllipsizeMode::End)
    ...
    progress.add_css_class("op-progressbar");
    progress.set_fraction(if op.duration_ms > 0 {
    ...
    controls.append(&mk("media-skip-backward-symbolic"));
    controls.append(&mk(
        if op.state == crate::music_operations::PlaybackState::Playing {
            "media-playback-pause-symbolic"
        } else {
            "media-playback-start-symbolic"
        },
    ));
    controls.append(&mk("media-skip-forward-symbolic"));
    controls.append(&mk("media-playback-stop-symbolic"));
    card.append(&controls);

    Some(card)
}

fn build_music_pending_widget() -> Option<gtk::Revealer> {
    let bar = build_music_undo_bar_widget();
    let card = build_music_card_snapshot()?;
    let overlay = gtk::Overlay::new();
    ...
    attach_ops_pending_swipe(
        ...
        &bar,
        &card,
        crate::music_operations::dismissed_dir().unwrap_or(1.0),
        Rc::new(|| {
            let _ = crate::music_operations::restore();
        }),
        Rc::new(crate::music_operations::stop_music),
    );
...
/// Build the "Now Playing" widget shown at the top of the Operations popover.
///
/// The card is wrapped in a `gtk::Revealer` (SlideUp/SlideDown) so dismissal
/// plays a slide-out animation before calling `on_stop`. The user can dismiss
/// by swiping left or right on the card, or by clicking the close (×) button.
/// `on_stop` is called after the animation finishes — the caller uses it to
/// show the undo bar. Returns `None` when nothing is playing.
fn now_playing_widget(on_stop: impl Fn() + 'static) -> Option<gtk::Box> {
    let op = crate::music_operations::current()?;
    let track = op.current_track()?.clone();
    let scroll_sign: f64 = if touchpad_natural_scroll() { -1.0 } else { 1.0 };

    // Outer container returned to the caller — always visible.
    let outer = gtk::Box::builder()
        .orientation(gtk::Orientation::Vertical)
        .build();

    // Revealer wraps the card and drives the slide-out animation.
    let revealer = gtk::Revealer::builder()
        .transition_type(gtk::RevealerTransitionType::SlideUp)
        .transition_duration(250)
        .reveal_child(true)
    ...
    let close_btn = gtk::Button::builder()
        ...
        .css_classes(["flat", "circular"])
        .valign(gtk::Align::Start)
        .tooltip_text("Swipe or click to dismiss")
        .build();
    top.append(&close_btn);
    card.append(&top);

    // Scrub bar: draggable/clickable seek. Range is 0..=duration (ms); a
    // zero duration falls back to a flat bar. User-initiated changes seek;
    // the refresh loop below keeps the value in sync when not dragging.
    let bar = gtk::Scale::builder()
        .orientation(gtk::Orientation::Horizontal)
        .adjustment(&gtk::Adjustment::new(
            0.0,
            0.0,
            op.duration_ms.max(1) as f64,
            1.0,
            5.0,
            0.0,
        ))
        ...
        .hexpand(true)
        .build();
    bar.add_css_class("spotty-scrub");
    bar.set_value(op.elapsed_ms.min(op.duration_ms.max(1)) as f64);
    card.append(&bar);

    // User-initiated scrub: seek. `change-value` only fires on user interaction
    // (drag/click/keys), not on programmatic set_value.
    let scrubbing = Rc::new(Cell::new(false));
    {
        let bar_c = bar.clone();
        let scrubbing_c = scrubbing.clone();
        bar.connect_change_value(move |_, _, value| {
            scrubbing_c.set(true);
            crate::music_operations::seek_to(value.max(0.0) as u64);
            let bar_c = bar_c.clone();
            let scrubbing_d = scrubbing_c.clone();
            // Clear the guard shortly after the interaction ends so the
            // refresh loop resumes writing the live position.
            glib::timeout_add_local_once(Duration::from_millis(300), move || {
                scrubbing_d.set(false);
                bar_c.set_sensitive(true);
            });
            glib::Propagation::Stop
        });
    }
    ...
    let prev_btn = mk("media-skip-backward-symbolic", "Previous");
    prev_btn.set_sensitive(op.index > 0);
    prev_btn.connect_clicked(|_| crate::music_operations::previous());
    let play_btn = mk("media-playback-start-symbolic", "Play/Pause");
    play_btn.connect_clicked(|_| crate::music_operations::toggle());
    let next_btn = mk("media-skip-forward-symbolic", "Next");
    next_btn.set_sensitive(op.index + 1 < op.queue.len());
    next_btn.connect_clicked(|_| crate::music_operations::next());
    let stop_btn = mk("media-playback-stop-symbolic", "Stop");
    // stop_btn is wired below, after the dismiss closure is defined.
    ...
    // Swipe hint label — faint, appears during drag.
    let swipe_hint = gtk::Label::builder()
        .label("← swipe to dismiss →")
        .css_classes(["dim-label", "caption"])
        .halign(gtk::Align::Center)
        .opacity(0.0)
        .build();
    card.append(&swipe_hint);

    // ── Real-time translateX via the shared swipe provider ────────────────
    let card_cls = format!("swrow{}", SWIPE_ROW_CTR.fetch_add(1, Ordering::Relaxed));
    card.add_css_class(&card_cls);
    let set_card_css = {
        let cc = card_cls.clone();
        move |tx: f64, transition: &str| swipe_apply(&cc, tx, transition)
    };

    // ── Shared dismiss closure ─────────────────────────────────────────────
    // Slides the card off in the swipe direction (300ms), collapses its height,
    // then pauses + hides the session (preserving position) and calls on_stop to
    // show the inline undo bar. `dir` is positive for a right-swipe, negative for
    // a left-swipe — it is recorded so a reverse swipe can resume playback.
    let on_stop = Rc::new(on_stop);
    let dismiss: Rc<dyn Fn(f64)> = {
        let set_css = set_card_css.clone();
        ...
        Rc::new(move |dir: f64| {
            ...
            glib::timeout_add_local_once(Duration::from_millis(310), move || {
                ...
                glib::timeout_add_local_once(Duration::from_millis(250), move || {
                    crate::music_operations::dismiss(dir);
                    on_stop();
                });
            });
        })
    ...
    // Guard: at most one dismiss fires (close btn, stop btn, drag, scroll).
    let dismissed = Rc::new(Cell::new(false));

    // Close button (×) — slide right.
    {
        let d = dismiss.clone();
        let dis = dismissed.clone();
    ...
    // ── Drag gesture: pointer/touch — card physically follows the drag ─────
    let gesture_drag = gtk::GestureDrag::new();
    gesture_drag.set_touch_only(false);
    gesture_drag.set_propagation_phase(gtk::PropagationPhase::Bubble);
    {
        let set_css = set_card_css.clone();
        let hint = swipe_hint.clone();
        let d = dismiss.clone();
        let dis = dismissed.clone();
        gesture_drag.connect_drag_update(move |_, dx, _dy| {
            if dis.get() {
                return;
            }
            set_css(dx, "");
            hint.set_opacity((dx.abs() / SWIPE_COMMIT_PX).clamp(0.0, 0.9));
            if dx.abs() >= SWIPE_OFFSCREEN_PX {
                dis.set(true);
                d(dx);
            }
    ...
    }
    {
        ...
        let d = dismiss.clone();
        let dis = dismissed.clone();
        gesture_drag.connect_drag_end(move |_, dx, _dy| {
            if dis.get() {
                return;
            }
            if dx.abs() >= SWIPE_COMMIT_PX {
    ...
    // Attach to the non-transformed revealer, not `card` (which gets a CSS
    // translateX): dragging the widget the gesture lives on would shift its own
    // coordinate frame and make the offset jitter.
    revealer.add_controller(gesture_drag);

    // ── Touchpad / horizontal-wheel swipe (matches the ops rows) ───────────
    let scroll_ctrl = gtk::EventControllerScroll::new(gtk::EventControllerScrollFlags::HORIZONTAL);
    ...
    let scroll_acc = Rc::new(Cell::new(0.0_f64));
    {
        ...
        let d = dismiss.clone();
        let dis = dismissed.clone();
        scroll_ctrl.connect_scroll(move |_, dx, _dy| {
            if dis.get() {
                return glib::Propagation::Stop;
            }
            if dx.abs() < 0.005 {
            ...
            let px = new_acc * SWIPE_SCALE;
            set_css(px, "");
            hint.set_opacity((px.abs() / SWIPE_COMMIT_PX).clamp(0.0, 0.9));
            if px.abs() >= SWIPE_COMMIT_PX {
                dis.set(true);
                d(px);
            }
    ...
    card.add_controller(scroll_ctrl);

    // ── Live refresh loop ──────────────────────────────────────────────────
    // Track the last-seen queue index so we can detect track changes in the
    // refresh loop and update cover + title accordingly.
    let last_index = std::rc::Rc::new(std::cell::Cell::new(op.index));
    let last_cover = std::rc::Rc::new(std::cell::RefCell::new(
        op.cover_path
            .clone()
    ...
    let bar_w = bar.downgrade();
    let sub_w = sub.downgrade();
    let play_w = play_btn.downgrade();
    let prev_w = prev_btn.downgrade();
    let next_w = next_btn.downgrade();
    let cover_w = cover.downgrade();
    let title_w = title_lbl.downgrade();
    let badge_w = badge.downgrade();
    let refresh = move || {
        let (Some(outer), Some(bar), Some(sub), Some(play)) = (
            outer_w.upgrade(),
            bar_w.upgrade(),
            sub_w.upgrade(),
            play_w.upgrade(),
        ...
        match crate::music_operations::current() {
            Some(op) => {
                if !scrubbing.get() {
                    let max = op.duration_ms.max(1) as f64;
                    bar.set_range(0.0, max);
                    bar.set_value(op.elapsed_ms.min(op.duration_ms.max(1)) as f64);
                }

                if let Some(t) = op.current_track() {
                    if op.index != last_index.get() {
                        last_index.set(op.index);
                        if let Some(lbl) = title_w.upgrade() {
                            lbl.set_label(&t.title);
                        }
                        if let Some(b) = badge_w.upgrade() {
                            b.set_label(if t.is_youtube {
                                "YouTube Music"
                            } else {
                                "Local"
                            });
                        ...
                        if let Some(c) = cover_w.upgrade() {
                            c.set_icon_name(Some("audio-x-generic-symbolic"));
                        }
                        *last_cover.borrow_mut() = String::new();
                    }
                    ...
                    let new_cover = op
                        ...
                        .map(|p| p.display().to_string())
                        .unwrap_or_default();
                    if new_cover != *last_cover.borrow() && !new_cover.is_empty() {
                        *last_cover.borrow_mut() = new_cover.clone();
                        if let Some(c) = cover_w.upgrade() {
                            if std::path::Path::new(&new_cover).exists() {
                                c.set_from_file(Some(&new_cover));
                            }
                        }
                    }
                ...
                let playing = op.state == crate::music_operations::PlaybackState::Playing;
                play.set_icon_name(if playing {
                    "media-playback-pause-symbolic"
                } else {
                    "media-playback-start-symbolic"
                });
                if let Some(p) = prev_w.upgrade() {
                    p.set_sensitive(op.index > 0);
                }
                if let Some(n) = next_w.upgrade() {
                    n.set_sensitive(op.index + 1 < op.queue.len());
                }
                // Keep polling until the outer widget is gone.
                outer.is_visible() || outer.parent().is_some()
            }
            None => {
...
fn upd_preview(row: &gtk::ListBoxRow, rs: &[SearchResult], p: &PreviewPane) {
    ...
    match &r.action {
        ...
        // Music results: cover art + source + view/like overview. The
        // now-playing transport row (icon "music-row") isn't a track to preview.
        Action::PlayMusic { source, title } => p.show_music(source, title),
...
fn activate(
    ...
    match &result.action {
        ...
        Action::EnterMode(s) => {
            // Activating the now-playing row toggles play/pause; the transport
            // buttons handle their own clicks. Otherwise mode entry is handled
            // in the key handler (which owns the chip + mode state).
            if s.starts_with("__music__") {
                crate::music_operations::toggle();
            }
        }
        ...
        Action::PlayMusic { source, title } => {
            dismiss(window, shown, false);
            log::info!("music: play requested: {} ({})", title, source);

            // Build a playback queue from every playable track currently shown,
            // so next/previous walk the visible list. The activated track sets
            // the starting index.
            let mut queue: Vec<crate::music::Track> = Vec::new();
            let mut index = 0;
            for r in results {
                if let Action::PlayMusic {
                    source: s,
                    title: t,
                } = &r.action
                {
                    ...
                    queue.push(crate::music::Track::from_action(s, t));
                }
            }
            if queue.is_empty() {
                queue.push(crate::music::Track::from_action(source, title));
            }
            crate::music_operations::play_queue(queue, index);
        }
    }
}
